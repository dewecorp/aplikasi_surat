...
function sims_api_out_json_keluar($code, $payload)
{
    global $conn;
    http_response_code($code);
    sims_log_api($conn, 'surat-keluar', $code, $payload['message'] ?? null);
    echo json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    exit;
}

function sims_api_key_ok_keluar($conn)
{
...
if (!sims_api_key_ok_keluar($conn)) {
    sims_api_out_json_keluar(401, ['status' => 'error', 'message' => 'API key tidak valid atau API nonaktif.']);
}
...
if ($updated_since !== '') {
    ...
    if ($dt === false || $dt->format('Y-m-d H:i:s') !== $updated_since) {
        sims_api_out_json_keluar(400, ['status' => 'error', 'message' => 'Format updated_since harus Y-m-d H:i:s.']);
    }
...
if (!empty($_GET['search'])) {
    ...
    $where[] = "(no_surat LIKE '%$s%' OR perihal LIKE '%$s%' OR penerima LIKE '%$s%')";
}
$limit = isset($_GET['limit']) ? max(0, min(1000, (int)$_GET['limit'])) : 0;

$has_updated = false;
$rc = @mysqli_query($conn, "SHOW COLUMNS FROM surat_keluar LIKE 'updated_at'");
...
$sql = 'SELECT id, tgl_surat, no_surat, jenis_surat, perihal, penerima, file, created_at FROM surat_keluar';
if ($where !== []) {
...
if (!$q) {
    sims_api_out_json_keluar(500, ['status' => 'error', 'message' => 'Query gagal.']);
}
